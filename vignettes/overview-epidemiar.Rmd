...
The modeling utilizes `mgcv::bam()`, so the model form can be any accepted by it - any quadractically penalized GLM with the extended families in family.mgcv also being available. This is user set with the `fc_family` parameter. For example, you can run regression with a Poisson distribution (`fc_family = "poisson()"`). 
...
<!-- thin plate splines <<>>
<<>> environmental lags per cluster with thin plate smooths
-->
...
Currently, epidemiar supports the Farrington improved algorithm for event detection, using `surveillance::Farringtonflexible()` by setting `report_settings$ed_method = "farrington"`. 
