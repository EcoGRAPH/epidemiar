...
The `run_validation()` function will call `run_epidemia()`, so it will also take all the arguments for that function. The user does not need to modify any of these arguments (e.g. event detection settings, `fc_future_period`), as `run_validation()` will automatically handle all of thse adjustments. 
...
For a formatted validation report, please look at the accompanying R project `epidemiar-demo` and the `run_validation_amhara.R` script in the validation folder, using the `epidemia_validation.Rnw` Sweave formatting script.
