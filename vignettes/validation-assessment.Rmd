...
* `skill_test`: TRUE/FALSE on whether to also run the null/naive models for comparison: Persistence - last known value carried forward n-appropriate weeks, and Average Week - the average cases from that week of the week (per geographic grouping). 
...
Results are returned in a list, by model (name of the base model and the null models if run with skill test comparison). 
